...
import { decodeStorageRecord, SessionId, type SessionEvent, type SessionHeader } from '@deepseek-ai/dsh-session'
 ...
 */
function readFirstFrame(file: string): Buffer {
  ...
  try {
    ...
    for (;;) {
      ...
      if (frames.length === 1) return buffer.subarray(frames[0].start, frames[0].end)
    }
    const { frames } = scanZstdFrames(buffer, 1)
...
export function sameCwd(a: string, b: string): boolean {
  const normalize = (value: string): string => {
    ...
    if (process.platform === 'win32') return value.replace(/[\\/]+$/, '').toLowerCase()
...
): { frames: Array<{ start: number; end: number }>; tornStart?: number } {
 ...
 */
function decodeSessionFile(file: string, firstFrameOnly: boolean): string {
  if (!file.endsWith('.zstd')) return readFileSync(file, 'utf8')
  if (firstFrameOnly) return zstdDecompressSync(readFirstFrame(file)).toString('utf8')
  ...
  if (!firstFrameOnly && tornStart !== undefined) {
    try {
      parts.push(
        zstdDecompressSync(buffer.subarray(tornStart), {
...
function readHeader(file: string): SessionHeader | undefined {
  ...
  return meta as unknown as SessionHeader
}
 ...
 */
function readSharedTitles(): Readonly<Record<string, string>> {
  try {
    const parsed = JSON.parse(
      readFileSync(join(dshHome(), 'storages', 'session_projcache.json'), 'utf8'),
    ) as unknown
    const sessions = (parsed as { tables?: { sessions?: unknown } })?.tables?.sessions
    ...
    for (const [id, entry] of Object.entries(sessions as Record<string, unknown>)) {
      const val = (entry as { rows?: { title?: { val?: unknown } } })?.rows?.title?.val
      if (typeof val === 'string' && val.length > 0) titles[id] = val
    }
    return titles
  } catch {
    return {}
 ...
 */
function sessionArtifact(dir: string): string | undefined {
 ...
 */
export function listSharedSessions(): SharedSessionEntry[] {
  ...
  const entries: SharedSessionEntry[] = []
 ...
 */
export function importSharedSession(sessionId: string): SharedSessionLog | undefined {
  const root = join(dshHome(), 'sessions')
  let slugs: string[]
  try {
    slugs = readdirSync(root)
  ...
  for (const slug of slugs) {
    ...
    try {
      ...
      return { meta: meta as unknown as SessionHeader, events }
    } catch {
      return undefined
    }
  }
 ...
 */
export async function ensureSharedSessionImported(
  persistence: ImportablePersistence | undefined,
  sessionId: string,
): Promise<'local' | 'imported' | 'missing'> {
  if (persistence?.loadStored === undefined || persistence.appendBatch === undefined) {
    return 'missing'
  }
  const id = SessionId(sessionId)
  try {
    if ((await persistence.loadStored(id)) !== undefined) return 'local'
  } catch {
    // Unknown to the local store — fall through to the import attempt.
  }
  const log = importSharedSession(sessionId)
  if (log === undefined) return 'missing'
  try {
