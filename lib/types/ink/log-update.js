    ...
    render(prev, next, altScreen = false, decstbmSafe = true) {
        ...
        if (shrinking) {
            ...
            if (!altScreen && prev.screen.height > prev.viewport.height) {
                return fullResetSequence_CAUSES_FLICKER(next, 'offscreen', this.options.stylePool, undefined, true);
            }
 ...
 */
visibleOnly) {
    const startY = visibleOnly === true && frame.screen.height > frame.viewport.height
