...
import { ensureSharedSessionImported } from './sharedSessions.js';
...
async function resolveAgent(ctx, requestedSessionId, agentOptions, meta, configuredPreset) {
    if (requestedSessionId !== undefined) {
        ...
        try {
            ...
            const imported = await ensureSharedSessionImported(ctx.get('sessionPersistence'), requestedSessionId);
            if (imported === 'imported') {
                try {
                    ...
                    ctx.logger.warn(`cc-tui: resume of imported session "${requestedSessionId}" failed: ${retryError instanceof Error ? retryError.message : String(retryError)}`);
                }
            }
