...
import { ensureSharedSessionImported, listSharedSessions, sameCwd } from './sharedSessions.js';
...
export function createChannel(ctx, initialAgent, options) {
    ...
    const state = {
        ...
        async resumeTo(sessionId) {
            ...
            // A session that lives only in the shared JSONL store (dsh web / dsh
            // CLI — issue #24) is materialized into cc-tui's SQLite store first, so
            // the resume below goes through the normal persistence seam.
            const imported = await ensureSharedSessionImported(ctx.get('sessionPersistence'), sessionId);
            if (imported === 'imported') {
                state.notify('已从 dsh 共享会话库导入该会话');
            }
            let handle;
        ...
        async listSessions() {
            ...
            try {
                ...
                // 目录启动的会话，别的项目的会话不出现在选择器里。Windows 上不同
                // 前端写入的盘符大小写/尾部分隔符可能不同，sameCwd 统一归一化。
                const cwd = state.cwd;
                const local = headers.filter(header => sameCwd(header.cwd ?? '', cwd));
                ...
                const visible = records.filter(record => !empty.has(record.id));
                // 合并 dsh web / dsh CLI 的共享 JSONL 会话库（issue #24）：cc-tui 自己
                // 的会话写在独立的 SQLite 库，web 会话写在 $DSH_HOME/sessions 下，此前
                // /resume 永远看不到它们。已在本地库出现的 id 跳过（本地库为准），其余
                // 按同样的 cwd 隔离过滤；标题用 web 侧 projection cache 维护的标题。
                const known = new Set(visible.map(record => String(record.id)));
                for (const shared of listSharedSessions()) {
                    if (known.has(shared.id) || !sameCwd(shared.cwd, cwd))
                        continue;
                    visible.push({
                        id: SessionId(shared.id),
                        title: shared.title ?? (basename(shared.cwd) || `session ${shared.id.slice(0, 8)}`),
                        cwd: shared.cwd,
                        createdAt: shared.createdAt,
                        updatedAt: lastUsed[shared.id] ?? shared.updatedAt,
                    });
                }
